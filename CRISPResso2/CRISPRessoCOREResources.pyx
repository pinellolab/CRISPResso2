...
cpdef find_indels_substitutions(read_seq_al, ref_seq_al, _include_indx):
    ...
    for idx_c, c in enumerate(ref_seq_al):
        if c != '-':  # is it safe to assume c will be '-' or in nucSet? This may be marginally faster than searching the set
            ...
            if start_insertion != -1:  # this is the end of an insertion
                ...
                if start_insertion in include_indx_set and idx in include_indx_set:
                    ...
                    insertion_sizes.append(idx - start_insertion)
                start_insertion = -1
            idx += 1
