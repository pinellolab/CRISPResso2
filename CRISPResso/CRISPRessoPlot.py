...
def plot_nucleotide_quilt(nuc_pct_df,mod_pct_df,fig_filename_root,save_also_png=False,sgRNA_intervals=None):

    if nuc_pct_df.iloc[1,5] > 1:
    ...
    min_text_pct = 0.5 #add text annotation if the percent is greater than this number
    max_text_pct = 0.95
            ...
            for nuc_ind in range(nNucs): #iterate over each nucleotide at this position in this sample
                ...
                if pct > min_plot_pct:
                    ...
                    if pct > min_text_pct and pct < max_text_pct:
                        ax.text(x_start+0.55,y_start + obs_pct/2.0,format(pct*100,'.1f'),horizontalalignment='center',verticalalignment='center',rotation=90,fontsize='smaller')
                    y_start += obs_pct
        ...
        for i in range(nSamples): #iterate over all samples
            ...
            if ins_pct > min_plot_pct:
                ...
                if ins_pct > min_text_pct and ins_pct < max_text_pct:
                    ax.text(x_start+0.5,y_start + obs_pct/2.0,format(ins_pct*100,'.1f'),horizontalalignment='center',verticalalignment='center',rotation=90)
...
def plot_subs_across_ref(ref_len, ref_seq, ref_name, ref_count, all_substitution_base_vectors, plot_title, fig_filename_root, save_also_png):
    """
    ...
    ax.set_ylim(0,(max(max(tots),1))*1.1) #max to avoid ylim of 0,0 which makes python freak out
...
def plot_log_nuc_freqs(df_nuc_freq,tot_aln_reads,plot_title,fig_filename_root,save_also_png):
    """
    Plots a heatmap of the percentage of reads that had each nucletide at each base in the reference
    Positions in the reference that have more than one allele can be spotted using this plot
    """
...
def plot_conversion_at_sel_nucs_not_include_nosub(df_subs,ref_name,ref_sequence,plot_title,conversion_nuc_from,fig_filename_root,save_also_png):
    '''
    ...
    ax2.set_xticklabels(["%0.2f%%"%(x*100)  for x in sub_freq] +["         $\it{\% Converted}$"])
...
def plot_alleles_heatmap(reference_seq,fig_filename_root,X,annot,y_labels,insertion_dict,per_element_annot_kws,SAVE_ALSO_PNG=False,base_editor_mode=False,sgRNA_intervals=None):
    """
    ...
    -base_editor_mode: if true, won't draw 'predicted cleavage' line
    """
    ...
    if len(per_element_annot_kws) > 1:
        ...
        print("Only one element -- not plotting allele plots")
        return
...
def plot_alleles_table(reference_seq,df_alleles,fig_filename_root,MIN_FREQUENCY=0.5,MAX_N_ROWS=100,SAVE_ALSO_PNG=False,base_editor_mode=False,sgRNA_intervals=None):
    """
    ...
    plot_alleles_heatmap(reference_seq,fig_filename_root,X,annot,y_labels,insertion_dict,per_element_annot_kws,SAVE_ALSO_PNG,base_editor_mode,sgRNA_intervals)

def plot_alleles_table_compare(reference_seq,df_alleles,sample_name_1,sample_name_2,fig_filename_root,MIN_FREQUENCY=0.5,MAX_N_ROWS=100,SAVE_ALSO_PNG=False,base_editor_mode=False,sgRNA_intervals=None):
    """
